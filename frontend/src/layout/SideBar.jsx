...
const SideBar = ({ isSideBar, setIsSideBar, setSelectedComponent }) => {
  ...
  return (
    <>
      <aside className={`${isSideBar?"left-0":"left-full"} z-10 transition-all duration-700 md:relative md:left-0 flex w-64 bg-black text-white flex-col h-full`} style={{position:"fixed"}}>
          <div className="px-6 py-6 flex justify-center items-center">
             <img src={bookstacktitlewhite} alt="Bookstack logo" className="w-40 h-auto object-contain"/>
          </div>
          <nav className="flex-1 px-6 space-y-2">
            ...
            { isAuthenticated && user?.role==="User" && (
                <>
                <button className="w-full py-2 font-medium bg-transparent rounded-md hover:cursor-pointer flex items-center space-x-2
                      " onClick={()=>setSelectedComponent("My Books")}>
                   <img src={catalogIcon} alt="icon"/>
                   <span>My Books </span>
                 </button>
                </>
