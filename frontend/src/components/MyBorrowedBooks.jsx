...
const MyBorrowedBooks = () => {
  ...
  useEffect(() => {
    ...
    const savedFilter = localStorage.getItem('myBooksFilter');
    if (savedFilter) {
      setFilter(savedFilter);
      // Clear the filter from localStorage after using it
      localStorage.removeItem('myBooksFilter');
    }
  }, []);

  const returnedBooks = userBooks?.filter(book=>{
    return book.returned===true;
  })
   const nonReturnedBooks = userBooks?.filter(book=>{
  ...
  return <>
     <main className="relative flex-1 p-6 pt-28">
        ...
        <header className="flex flex-col gap-3 sm:flex-row md:items-center">
          <button className={`relative rounded sm:rounded-tr-none sm:rounded-br-none sm:rounded-tl-lg sm:roundede-bl-lg text-center border-2 font-semibold py-2 w-full sm:w-72 ${filter==="returned"? "bg-black text-white border-black":"bg-gray-200 text-black border-gray-200 hover:bg-gray-300"}`}  onClick={()=>setFilter("returned")} >
                 Returned Books
          </button>
          <button className={`relative rounded sm:rounded-tl-none sm:rounded-bl-none sm:rounded-tr-lg sm:roundede-br-lg text-center border-2 font-semibold py-2 w-full sm:w-72 ${filter==="nonReturned"? "bg-black text-white border-black":"bg-gray-200 text-black border-gray-200 hover:bg-gray-300"}`}  onClick={()=>setFilter("nonReturned")} >
                 Non-Returned Books
          </button>
        </header>
          {
            booksToDisplay && booksToDisplay.length>0?(
                 <div className="mt-6 overflow-auto bg-white rounded-md shaddow-lg">
                  <table className="min-w-full border-collapse">
                    <thead>
                      ...
                      <th className="px-4 py-2 text-left">DUE BY</th>
                    ...
                    <tbody>
                      {
                        booksToDisplay.map((book,index)=>(
                          <tr key={index} className={(index+1)%2===0?"bg-gray-50":""}>
                            ...
                            <td className="px-4 py-2">{getBookTitle(book.bookId)}</td>
